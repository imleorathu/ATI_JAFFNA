...
const extensionGroups = {
  assignment: new Set(["pdf", "doc", "docx", "ppt", "pptx", "xls", "xlsx", "csv", "txt", "zip", "png", "jpg", "jpeg", "webp"]),
...
function createFileFilter(groupName) {
  return (_req, file, callback) => {
    ...
    if (allowedExts.has(ext) && allowedMimes.some((pattern) => pattern.test(file.mimetype))) {
      callback(null, true);
      return;
    }

    const error = new Error(`Unsupported file type. Allowed extensions: ${[...allowedExts].join(", ")}.`);
    error.status = 400;
...
function createDiskUpload({ uploadDir, groupName, maxFileSize, filenamePrefix }) {
  return multer({
    storage: multer.diskStorage({
      destination: (_req, _file, callback) => callback(null, uploadDir),
      filename: (req, file, callback) => {
        const prefix = typeof filenamePrefix === "function" ? filenamePrefix(req, file) : filenamePrefix;
        callback(null, `${prefix || Date.now()}-${safeUploadName(file.originalname)}`);
      }
    }),
